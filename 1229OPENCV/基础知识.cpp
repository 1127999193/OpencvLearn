...
int main1()
{
	//Mat 图像初始化操作  都拥有自己独立的内存空间
	Mat img(100,100,CV_8UC3,Scalar(0,0,255));
	//imshow("img", img);

	Mat img1;
	img1.create(100, 100,CV_8UC3);
	//imshow("img1", img1);

	Mat img2 = Mat::zeros(100,100,CV_8UC3);
	//imshow("img2", img2);
	...
	threshold(src_gray,dst_th,150,255,CV_THRESH_BINARY);
	//imshow("thre", dst_th);
	//imshow("src", src);

	//CV_ADAPTIVE_THRESH_MEAN_C  平均取得权值
	//CV_ADAPTIVE_THRESH_GAUSSIAN_C  高斯计算权值

	Mat dst_adath;
	adaptiveThreshold(src_gray,dst_adath,255, CV_ADAPTIVE_THRESH_GAUSSIAN_C, CV_THRESH_BINARY_INV,21,0);
	//imshow("dst_adath", dst_adath);


	//腐蚀、膨胀
	//腐蚀和膨胀是对白色（高亮部分）而言的，膨胀是对图像中高亮部分进行膨胀，
	//效果图拥有比原图更大的的高亮区域
	//腐蚀是对原图高亮部分的腐蚀，效果图拥有比原图更小的高亮区域

	//getStructuringElement 函数返回指定的形状和尺寸的结构元素
	...
	Mat dst_canny;
	Canny(dst_blur, dst_canny,6,9);

	//imshow("dst_canny", dst_canny);

	//WINDOW_NORMAL 可以允许用户自由伸缩窗口大小
	namedWindow("test",WINDOW_NORMAL);

	Mat dst_equa;
	//直方图均衡化的图片对比度高，更加明亮
	equalizeHist(src_gray,dst_equa);
	//imshow("dst_equa", dst_equa);
